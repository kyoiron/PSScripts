...
#MSI檔安裝路徑
$AdobeReader64_EXE_Folder="\\172.29.205.114\loginscript\Update\AdobeAcrobatDC(64-bit)"
#沒裝的是否要裝$true或$false
...
<#
    ...
    Get-MsiInformation  AcroPro.msi檔結果
    File            : \\172.29.205.114\loginscript\Update\AdobeAcrobatDC(64-bit)\AcroPro.msi
    ProductCode     : {AC76BA86-1028-1033-7760-BC15014EA700}
    Manufacturer    : Adobe
    ProductName     : Adobe Acrobat DC (64-bit)
    ProductVersion  : 21.001.20135
    ProductLanguage : 1028
#>    
if($MSP -and $AcroRead_MSI -and $MSP64 -and $AcroRead64_MSI){    
    ...
    $AdobeReader64_installeds = Get-ItemProperty $RegUninstallPaths[0] | Where-Object{$_.PSChildName -eq $AcroRead64_MSI.ProductCode} #| ForEach-Object{ Uninstall-MSI ($($_.UninstallString))}
    <#
        AuthorizedCDFPrefix : 
        Comments            :                           			
        Contact             : 客戶支援
        ...
        Contact             : 客戶支援
        DisplayVersion      : 22.002.20212
        HelpLink            : http://www.adobe.com/tw/support/main.html
        HelpTelephone       : 
                              			
        InstallDate         : 20220912
        InstallLocation     : C:\Program Files\Adobe\Acrobat DC\
        InstallSource       : C:\Program Files\Common Files\Adobe\Acrobat\Setup\{AC76BA86-1028-1033-7760-BC15014EA700}\
        ModifyPath          : MsiExec.exe /I{AC76BA86-1028-1033-7760-BC15014EA700}
        NoRepair            : 1
        Publisher           : Adobe
        Readme              : C:\Program Files\Adobe\Acrobat DC\ReadmeCT.htm
        Size                : 
        EstimatedSize       : 1052136
        UninstallString     : MsiExec.exe /I{AC76BA86-1028-1033-7760-BC15014EA700}
        URLInfoAbout        : http://www.adobe.com
        URLUpdateInfo       : http://helpx.adobe.com/tw/acrobat.html
        VersionMajor        : 22
        VersionMinor        : 2
        WindowsInstaller    : 1
        Version             : 369250036
        ...
        sEstimatedSize2     : 526068
        PSPath              : Microsoft.PowerShell.Core\Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\{AC76BA86-1028-1033-7760-BC15014EA700}
        PSParentPath        : Microsoft.PowerShell.Core\Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall
        PSChildName         : {AC76BA86-1028-1033-7760-BC15014EA700}
        PSDrive             : HKLM
        PSProvider          : Microsoft.PowerShell.Core\Registry
        
        

        File            : C:\Program Files\Common Files\Adobe\Acrobat\Setup\{AC76BA86-1028-1033-7760-BC15014EA700}\AcroPro.msi
        ProductCode     : {AC76BA86-1028-1033-7760-BC15014EA700}
        Manufacturer    : Adobe
        ProductName     : Adobe Acrobat DC (64-bit)
        ProductVersion  : 21.001.20135
    ...
    $AdobeReader64_installed = $AdobeReader64_installeds | Sort-Object -Property DisplayVersion -Descending | Select-Object -first 1
    if(($AdobeReader_installed -eq $null) -and ($AdobeReader64_installed -eq $null) -and ($Install_IF_NOT_Installed -ne $true)){exit}
    ...
    if($AdobeReader_installed -ne $null){   
        #有安裝Adobe Acrobat Reader DC - Chinese Traditional的狀況----進行更新
        ...
        if(!$AdobeReader64_installed){            
            ...
            #有安裝64bit版的狀況----進行更新         
            $MSP64_Version_String = $MSP64.BaseName.TrimStart("AcroRdrDCx64Upd")                                                            
            $MSP64_Version = $MSP64_Version_String.Substring(0,2)+"."+ $MSP64_Version_String.Substring(2,3)+"."+$MSP64_Version_String.Substring(5,5)         
            if([version]$AdobeReader64_installed.DisplayVersion -ge [version]$MSP64_Version){exit}
            $LogName = $env:Computername + "_"+ $AcroRead64_MSI.ProductName +"_"+ $MSP64_Version + ".txt" 
            $Log_Folder_Path = $Log_Path +"\"+ $AcroRead64_MSI.ProductName  
            if((Test-Path ($AdobeReader64_installed.InstallSource+"AcroPro.msi")) -and ($AdobeReader64_installed.VersionMajor -eq ([version]$MSP64_Version).Major)){
                ...
                if($Setup64_Content -match 'PATCH=AcroRdrDCx64Upd') {
                    ($Setup64_Content -replace ($Setup64_Content -match 'PATCH=AcroRdrDCx64Upd') , ("PATCH="+ $MSP64.Name) ) | Set-Content  "$env:systemdrive\temp\setup.ini"  
                }
            }
            robocopy $AdobeReader64_EXE_Folder "$env:systemdrive\temp" $MSP64.Name "/XO /NJH /NJS /NDL /NC /NS".Split(' ') | Out-Null
            $arguments = "/i ""$AcroRead64_MSI_Path"" /update "+ $env:systemdrive+"\temp\" + $MSP64.Name +" /qn /norestart /log ""$env:systemdrive\temp\$LogName"""    
            unblock-file ($env:systemdrive+"\temp\" + $MSP64.Name)
            start-process "msiexec" -arg $arguments -Wait    
            if(!(Test-Path -Path $Log_Folder_Path)){New-Item -ItemType Directory -Path $Log_Folder_Path -Force}
            if(Test-Path -Path "$env:systemdrive\temp\$LogName"){robocopy "$env:systemdrive\temp" $Log_Folder_Path $LogName "/XO /NJH /NJS /NDL /NC /NS".Split(' ') | Out-Null}
        }        
