...
<#
$InstallPrinterPC=@("TND-COU-043","TND-COU-048")
#指定哪個電腦的匯出當匯入範本
$ImportFromeComputername = "TND-COU-055"
#>
#$InstallPrinterPC=@("TND-GCSE-080")
#指定哪個電腦的匯出當匯入範本
#$ImportFromeComputername = "TND-GCSE-081"
...
#進行WindowsUpdate
    ...
    } -ArgumentList $temp,$LogPath
    Get-WUHistory | Format-Table -AutoSize | Out-File "$temp\${env:computername}_WindowsUpdate_History.txt" -Force
    Robocopy $temp $LogPath "${env:computername}_WindowsUpdate.txt" "${env:computername}_WindowsUpdate_History.txt" "/XO /NJH /NJS /NDL /NC /NS".Split(' ')| Out-Null
    #Robocopy $temp $LogPath "${env:computername}_WindowsUpdate_History.txt" "/XO /NJH /NJS /NDL /NC /NS".Split(' ')| Out-Null
<#
