...
<#
    ...
    IsSystem     : False   
#>
if($Chrome_MIS_FileMetaData -ne $null){
    ...
    $Chrome_MIS_fileName = Split-Path -Path $Chrome_MIS_FileMetaData.File -Leaf
    $arguments = "/i $env:systemdrive\temp\$Chrome_MIS_fileName /qn ALLUSERS=1 /L*VX ""$env:systemdrive\temp\" +  $LogName+""""
    ...
    if((Get-Process -Name Chrome -ErrorAction SilentlyContinue) -ne $null){ 
    ...
    #if(Test-Path -Path ("$env:systemdrive\temp\$Chrome_MIS_fileName")){Remove-Item -Path  "$env:systemdrive\temp\$Chrome_MIS_fileName" -Force}
